...
function SingleArticle() {
    ...
    const fetchArticle = async () => {
      try {
        ...
        if (id) {
          ...
          const slugQuery = query(articlesRef, where("seo.slug", "==", id));
    ...
    return (
      <div className="px-4 py-8">
        <div className="flex flex-col items-center justify-center py-24 bg-white border border-slate-200 rounded-xl">
          ...
          <p className="text-lg font-semibold text-slate-500 mt-4">
            Article not found
          </p>
          <button
  ...
  const metaTitle = article.seo?.metaTitle || article.title || "Article";
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="keywords"
          content={article.seo?.keywords || "articles, cms"}
        />
      ...
      <div className="px-4 py-6 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between bg-white border border-slate-200 rounded-xl px-4 py-2.5 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span
                className={`w-1 h-1 rounded-full ${isPublished ? "bg-emerald-500" : "bg-amber-500"}`}
              />
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {article.tags.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center px-2 py-1 text-[10px] font-medium bg-slate-100 text-slate-600 rounded-md border border-slate-200"
                      >
                        {tag}
                      </span>
                    ))}
